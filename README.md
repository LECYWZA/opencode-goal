...
| `thinking_stall_s` | `1800` | 死循环检测：持续输出但长时间不动手(秒=30分钟, 保险) → 思考空转死循环 |
...
设计目标：**无论多少会话、多少 opencode 实例同时启用目标，都不得冲突。** 两层机制保证：### A) 持久化合并写 + 排他写锁（防数据丢失）
...
>
> **辅助提示（机制辅助）**：只要是存在并行风险的模式（`parallel` 或 `serial+N`，N>1），插件会自动注入两路提示：
- **静态准则**（系统提示）：改现有文件前先读最新内容再改、分工尽量避重、每次改动后用 `git status/diff` 检查是否覆盖其它会话的修改、发现文件被他人改动时先重读再应用而非强制覆盖。
- **动态数据驱动清单**（每轮续跑指令）：插件实时监听 `file.edited` 事件，把**最近 30 秒内被改动过的文件**动态附在下一轮指令里，明确提示"这些文件可能刚被其它会话/实例改过，操作前先 read 最新内容再 Edit"。这是真实冲突风险信号，而非泛泛提醒。

串行（默认）模式不注入以上任何提示，避免噪音；这些是引导提示而非硬约束。说明：即便有辅助提示，`parallel` 或 `serial+N(N>1)` 下多个会话同时改**同一个文件**仍可能互相覆盖，放开并行是固有权衡，提示只尽量降低风险。
>
> 说明：即便有辅助提示，`parallel` 或 `serial+N(N>1)` 下多个会话同时改**同一个文件**仍可能互相覆盖，放开并行是固有权衡，提示只尽量降低风险。

## 死循环检测（防"思考/回复死循环"）

模型陷入"思考空转 / 重复输出 / 重复工具调用"时，插件会**自动发现并中断当前轮，然后进入下一轮迭代**，并在下一轮指令里提示模型"上一轮疑似死循环，请直奔能产生确定进展的动作"。三类信号：

| 信号 | 触发 | 配置项 |
|---|---|---|
| 思考空转 | 模型持续输出 token，但长时间（默认30分钟）没有任何工具动作 → thinking-stall | `thinking_stall_s` |
| 重复输出 | 连续输出完全相同内容达到次数（默认5）→ repeat-output | `max_repeats` |
| 工具循环 | 连续相同(工具+参数)调用达到次数（默认5）→ tool-loop | `max_tool_loop` |

被判定为死循环的轮 **不计入进展**（`noProgressTurns` 会计数并计入 recovery），即不会被误当作"干了很多活"。中断后默认进入自我进化恢复轮；在 `recovery=pause` 模式下则暂停。这些是机制级检测，非仅提示。

## 健康/并发可视化（`goal_overview`）

`goal_overview` 工具返回项目级监督信息：
- **当前活跃的 objective 会话**（每个 worktree 上的 sessionID、mode、objective、轮数、是否暂停/完成、最近结束状态、是否检测到死循环）；
- **最近 2 分钟内被改动过的文件**（来自 `file.edited` 事件，用于并行冲突监督）；
- **当前 worktree 的"运行权"归属**（本进程 / 其它进程 / 串行策略与并行度）。
