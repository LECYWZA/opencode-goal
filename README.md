...
>
> **辅助提示（机制辅助）**：只要是存在并行风险的模式（`parallel` 或 `serial+N`，N>1），插件会自动向模型系统提示与每轮指令注入"并发安全协作准则"——改现有文件前先读最新内容再改、分工尽量避重、每次改动后用 `git status/diff` 检查是否覆盖了其它会话的修改、发现文件被他人改动时先重读再应用而非强制覆盖。串行（默认）模式不注入，避免噪音；这些是引导提示而非硬约束。
>
