...
如果你的目标文本里**没有**带内联参数（支持：`agent=3`、`max_turns=100`、`recovery=auto-research|pause|continue`、`recovery_attempts=5`、`worktree_policy=serial|parallel`、`worktree_parallel_sessions=3`、`turn_timeout_s=120`、`converge_turns=5`、`no_progress_turns=10`、`idle_interval_ms=2000`），请**先逐一用 question 工具弹出选择让用户确认**下面每一项（不要擅自假设）：
...
5. 同项目并发策略：建议 `["串行(同一目录互斥,防冲突,默认)", "串行但同实例内N路并行(可再问N)", "并行(彻底不互斥)"]`（对应 worktree_policy=serial / serial+N / parallel，单选）

然后把「用户明确给出或选择」的所有参数连同目标传给 goal_set：
`goal_set(goal=<目标>, mode="iterate", agent=?, max_turns=?, recovery=?, worktree_policy=?, turn_timeout_s=?, converge_turns=?, ...)`，然后立刻开始第一轮。
